...
Having the models trained to convergence did significantly reduce the performance difference between simpler vocabularies and more complicated vocabularies. The benchmark results for both SMLQA (Ground Truth) and SQuAD (Data Extration) are very close. To close in fact, re-running the benchmark gives a different "best" vocabulary. The main difference is that 50256-consistent has a whopping 23.5% advantage with chr/tok. There is however, a small performance cost on Ground Truth associated with the vocabularies that use multiple words per token.
