package main //package tokenmonster
...
func cpuTime() (userTime, systemTime time.Duration) {
	var rusage syscall.Rusage
	if err := syscall.Getrusage(syscall.RUSAGE_SELF, &rusage); err != nil {
		fmt.Printf("Rusage error: %v", err)
	}
...
func main() {
	...
	vocabfilename := os.Args[1]
	testfilename := os.Args[2]

	vocab, err := LoadVocabFromFile(vocabfilename)
	if err != nil {
		panic(err)
	}
	...
	startUserTime, startSystemTime := cpuTime()

	res, _, err := vocab.Tokenize(data)
	if err != nil {
		panic(err)
	}

	endUserTime, endSystemTime := cpuTime()

	fmt.Println(testfilename, `has`, len(res), `tokens`)
